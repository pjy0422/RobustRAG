...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="llama7b",
        choices=["mistral7b", "llama7b", "gpt3.5", "llama8b"],
        help="model name",
    )
    parser.add_argument(
        "--dataset_name",
        ...
        choices=[
            ...
            "open_nq",
            "biogen",
            "NQ_SH_adv_2_source_5",
            "NQ_SH_adv_3_source_5",
            "num_src_3",
            "num_src_4",
            "num_src_5",
            "num_src_6",
    ...
    parser.add_argument("--num_src", type=int, default=4, help="number of sources")
    # attack
    parser.add_argument(
        "--attack_method",
        type=str,
        ...
        help="The attack method to use (Poison or Prompt Injection)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="keyword",
        choices=["none", "voting", "keyword", "decoding"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="keyword filtering threshold alpha",
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=3.0,
        help="keyword filtering threshold beta",
    )
    parser.add_argument(
        "--eta",
        type=float,
        default=0.0,
        help="decoding confidence threshold eta",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="The corruption size when considering certification/attack",
    )
    parser.add_argument(
        "--subsample_iter",
        type=int,
        default=1,
        help="number of subsampled responses for decoding certifictaion",
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="output debugging logging information",
    )
    parser.add_argument(
        "--save_response",
        action="store_true",
        help="save the results for later analysis",
    )
    parser.add_argument(
        "--use_cache",
        action="store_true",
        help="save/use cache responses from LLM",
    )
    parser.add_argument(
        "--no_vanilla", action="store_true", help="do not run vanilla RAG"
    )
    parser.add_argument("--data_path", type=str, default="", help="data path")
    parser.add_argument("--num_spammer", type=int, default=0, help="num_spammer")
...
def main():
    ...
    # load data
    data_tool = load_data(args.dataset_name, args.top_k, data_path=args.data_path)
    ...
    if args.defense_method == "voting":  # majority voting
        ...
        model = KeywordAgg(
            ...
            longgen=longgen,
            certify_save_path=certify_save_path,
        )
    ...
    if no_attack:
        ...
        if args.dataset_name == "biogen":
            attacker = PIALONG(
                top_k=args.top_k,
                poison_num=args.corruption_size,
                repeat=3,
                poison_order="backward",
            )
        else:
            attacker = PIA(
                top_k=args.top_k,
    ...
    if not no_attack:
        args.corruption_size = (
            0  # no certification for attack    # ad-hoc implementation -- tofix
        )
    ...
    response_list = []
    total = 0
    assert len(data_tool.data) == 1600
    data_tool.data = data_tool.data[200:]
    for data_item in tqdm(data_tool.data):
        ...
        #     {"query": data_item["question"], "undefended": response_undefended}
        # )

        # defended
        if not no_defense:
            response_defended, certificate = model.query(
                data_item, corruption_size=args.corruption_size
            )
            ...
            response_list.append(
                {
                    ...
                    "response": response_defended,
                    "true_answer": data_item["answer"],
                    "is_correct": defended_corr,
                }
            )
            corr_list.append(defended_corr and certificate)
        total += 1
        logger.info(f"지금 {total}개중에 {defended_corr_cnt}개 맞음")
        logger.info(f"지금 {total}개까지 EM: {defended_corr_cnt/total}")
        # save for later analysis, currently used for biogen dataset
        if args.save_response:
            temp_list = {
                "Length": total,
                "correct": defended_corr_cnt,
                "EM": defended_corr_cnt / total,
    ...
    # 파일 경로 정의
    # file_path = f"result/num_src_{args.num_src}_{args.defense_method}.csv"
    file_path = f"result/num_src_{args.num_src}_{args.defense_method}_num_spammer_{args.num_spammer}.csv"
    # 파일 존재 여부 확인 및 처리
    if not os.path.exists(file_path):
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, "w", newline="") as file:
            writer = csv.DictWriter(file, fieldnames=["iteration", "EM"])
            writer.writeheader()

    # 파일 열고 데이터 추가
    row_count = 0
    if os.path.exists(file_path):  # 파일이 존재하는 경우
        with open(file_path, "r", newline="") as file:
            reader = csv.reader(file)
            row_count = sum(1 for row in reader)  # 총 줄 수 계산

    with open(file_path, "a", newline="") as file:
        fieldnames = ["iteration", "EM"]
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        if row_count == 0:  # 파일이 비어 있다면 헤더를 작성
            writer.writeheader()
        writer.writerow({"iteration": args.iteration, "EM": defended_corr_cnt / total})
