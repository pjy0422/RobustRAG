...
def get_log_name(args):
    if args.defense_method == "none":
        ...
        defense_str = f"{args.defense_method}"
    elif args.defense_method == "keyword":
        defense_str = f"{args.defense_method}-{args.alpha}-{args.beta}"
    elif args.defense_method == "decoding":
        defense_str = f"{args.defense_method}-{args.eta}-{args.subsample_iter}"
    else:
        defense_str = ""

    if args.num_spammer == 0:
        return f"{args.dataset_name}-{args.model_name}-{defense_str}-top{args.top_k}-corr{args.corruption_size}-attack{args.attack_method}-num_src{args.num_src}-iteration{args.iteration}"
    else:
        return f"{args.dataset_name}-{args.model_name}-{defense_str}-top{args.top_k}-corr{args.corruption_size}-attack{args.attack_method}-num_src{args.num_src}-iteration{args.iteration}-num_spammer{args.num_spammer}"
...
from torch import FloatTensor, LongTensor, device, eq
...
class StopOnTokens(StoppingCriteria):
    ...
    def __call__(
        ...
        for stop_ids in self.stop_token_ids:
            if eq(
                input_ids[0][-len(stop_ids[0]) + 1 :], stop_ids[0][1:]
            ).all():
                return True
        return False
