...
class BaseModel:
    ...
    def wrap_prompt(self, data_item, as_multi_choice=True, hints=None, seperate=False):
        ...
        if as_multi_choice:
            mode += "-mc"
        if "long_gen" in data_item:
            mode += "-long"
        if not use_retrieval:
            mode += "-zero"
        if "decode" in data_item:
            mode += "-decode"
        if "genhint" in data_item:
            mode += "-genhint"
        if hints is not None:
            mode += "-hint"
        ...
        if seperate:
            return [
                ...
                for context_str in topk_content
            ]
        else:
            context_str = "\n\n".join(topk_content)
...
class HFModel(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.max_output_tokens = (
            MAX_NEW_TOKENS if max_output_tokens is None else max_output_tokens
        )
    ...
    def _query(self, prompt):
        ...
        if token_length >= (
            CONTEXT_MAX_TOKENS[self.model_name] - self.max_output_tokens
        ):
            prompt_length = len(prompt)
            ratio = (
                CONTEXT_MAX_TOKENS[self.model_name] - self.max_output_tokens - 100
            ) / token_length  # this is a rough cut off, 100 is a buffer
            ## this is left cut
            ...
            logger.warning(
                f"Prompt length exceeds the limit, cut the prompt to {inputs.input_ids.size(1)} tokens"
            )
