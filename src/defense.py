...
from .helper import StopOnTokens, clean_str
...
def save_all_responses(save_path, response_list, data_item):
    all_data = (
        []
    )  # it is a bit ugly... unnecessary read and write ; TODO: change it to jsonl instead
...
class RRAG:
    ...
    def query_undefended(self, data_item):
        query_prompt = self.llm.wrap_prompt(
            data_item, as_multi_choice="choices" in data_item
        )
        # response = None
    ...
    def query(self, data_item, corruption_size=0):
        ...
        seperate_responses = self.llm.batch_query(
            self.llm.wrap_prompt(
                data_item, as_multi_choice=True, seperate=True
            )
        )
        seperate_preds = []
        ...
        del cntr[
            "E."
        ]  # do not count E. # we still append 'E.' to `seperate_preds` because it is useful for certification
        ...
        if len(cntr) == 0:
            ...
            if INJECTION:
                delta -= sum(
                    [pred == x for x in seperate_preds[-corruption_size:]]
                )
                certificate = (
                    delta > corruption_size
                )  # conservatively consider tie as non-robust -- can be improved later
            else:
...
class KeywordAgg(RRAG):
    ...
    def __init__(
        ...
        self.certify_save_path = (
            certify_save_path  # save all possible responses if needed
        )
        logger.info(
            f"abs: {absolute_threshold}, relative: {relative_threshold}"
        )
    ...
    def query(self, data_item, corruption_size=0, abstention_threshold=None):
        ...
        abstention_threshold = (
            ...
            else self.abstention_threshold
        )
        if self.longgen:
            data_item["genhint"] = (
                True  # add a flag so that wrap_prompt() can retrieve the correct prompt template
            )
        # make seperate predictions
        seperate_responses_raw = self.llm.batch_query(
            self.llm.wrap_prompt(
                data_item, as_multi_choice=False, seperate=True
            )
        )
        abstained_idx = []
        seperate_responses = []
        ...
        logger.debug(
            f"Number of retained responses: {len(seperate_responses)}"
        )
        ...
        for response in seperate_responses:
            ...
            for token in doc:
                if token.pos_ in self.ignore_set:
                    if len(tmp) > 0:
                        phrase = "".join(
                            [x.lemma_ + x.whitespace_ for x in tmp]
                        ).strip()
        ...
        # filtering
        count_threshold = min(
            self.absolute, self.relative * len(seperate_responses)
        )
        logger.debug(
            sorted(
                token_counter.items(),
        ...
        for token, count in list(token_counter.items()):
            if (
                ...
                or (token in stopword_set)
                or (self.longgen and " " not in token)
            ):  # if it is long generation, we remove single words to reduce the size the keyword set...
        ...
        query_prompt = self.llm.wrap_prompt(
            data_item, as_multi_choice=False, hints=hints
        )
        ...
        if (
            corruption_size > 0
        ):  # only do certification when corruption is larger than zero
            if self.longgen or self._eval_response(
                response, data_item
            ):  # we don't need to do certification if it is a QA task (eval by correctness) and the clean response is incorrec
                for idx in abstained_idx:
                    all_extracted_phrase.insert(idx, set())
                certify_flg = self.certify(
                    data_item, corruption_size, all_extracted_phrase
                )
    ...
    def certify(self, data_item, corruption_size, all_extracted_phrase):
        ...
        if INJECTION:
            ...
            count_threshold = min(
                self.absolute, self.relative * (non_abs_cnt + corruption_size)
            )
            if (
                count_threshold <= corruption_size
            ):  # the attacker can introduce any keyword. return false
                return False
        else:
            non_abs_cnt = sum([len(x) > 0 for x in all_extracted_phrase])
            # slightly different from the paper, we can prove that k'_effective == corruption_size (k') generates keyword sets W^* that cover any k'_effective < corruption_size (k')
            count_threshold = min(
                self.absolute,
                self.relative * (max(corruption_size, non_abs_cnt)),
            )
            if (
                count_threshold <= corruption_size
            ):  # the attacker can introduce any keyword. return false
        ...
        for (
            ...
            all_extracted_phrase
        ):  # get the token counter for non-corrupted responses
        ...
        if INJECTION:
            ...
            added_list = [
                token
                for token, count in token_counter.items()
                if count_threshold - corruption_size <= count < count_threshold
            ]
        else:
            base_list = [
                token
                for token, count in token_counter.items()
                if count >= count_threshold + corruption_size
            ]
            added_list = [
                token
                for token, count in token_counter.items()
                if count_threshold - corruption_size
                <= count
                < count_threshold + corruption_size
            ]
        ...
        if len(added_list) > 14 or (self.longgen and len(added_list) >= 10):
            ...
            list_powerset = [
                base_list + added_list for added_list in added_list_powerset
            ]

            # sort the lists inside list_powerset by the length of the items in the list
            list_powerset = [
                sorted(lists, key=lambda x: (len(x), x), reverse=True)
                for lists in list_powerset
            ]
            hints_list = [
                ", ".join([f"{token}" for token in lists])
                for lists in list_powerset
            ]
            ...
            for i in range(0, len(prompt_list), batch_size):
                ...
                for response in response_batch:
                    if (
                        self.longgen
                    ):  # we do not do correctness evaluation. we need to save all possible responses
                        response_list.append(response)
                    else:
                        if not self._eval_response(
                            response, data_item
                        ):  # as long as one possible response is incorrect, we return false.
                            return False
            certify_flg = True
        ...
        if self.longgen:  # we need to dump all possible responses
            save_all_responses(
                self.certify_save_path, response_list, data_item
            )
...
def secure_decoding(
    ...
    use_cache: Optional[bool] = False,  # Set use_cache to False
    **model_kwargs,
):
    ...
    # Initialize attention mask and past_key_values in model_kwargs
    ...
    while True:
        ...
        next_token_confidence = torch.nn.functional.softmax(
            new_logits / temperature, dim=-1
        )
        ...
        if ii == 1:
            ...
            I_token = tokenizer("I", return_tensors="pt")["input_ids"][0][
                1
            ].item()
            n_token = tokenizer("/n", return_tensors="pt")["input_ids"][0][
                1
            ].item()
            Please_token = tokenizer("Please", return_tensors="pt")[
                "input_ids"
            ][0][1].item()
            next_token_agg[I_token] = (
                0  # set the probability of the "I" token to 0
            )
            next_token_agg[n_token] = (
                0  # set the probability of the "/n" token to 0
            )
            next_token_agg[Please_token] = (
                0  # set the probability of the "Please" token to 0
            )
        ...
        input_ids = torch.cat(
            (input_ids, selected_tokens.repeat(input_ids.shape[0], 1)), dim=-1
        )
        ...
        if all(stopping_criteria(input_ids, None)):
            break
...
def secure_decoding_certify(
    ...
    **initial_model_kwargs,
    ...
    if isinstance(eos_token_id, int):
        eos_token_id = [eos_token_id]
    eos_token_id_tensor = (
        torch.tensor(eos_token_id).to(initial_input_ids.device)
        if eos_token_id is not None
        else None
    )
    ...
    stack = [
        (
            0,
            torch.tensor(
                [], dtype=torch.long, device=initial_input_ids.device
            ),
        )
    ]  # Added None for the initial cache

    all_outputs = []
    certify = True
    past_key_values = None

    # Initialize attention mask and past_key_values in initial_model_kwargs
    initial_model_kwargs["use_cache"] = use_cache
    if "attention_mask" not in initial_model_kwargs:
        initial_model_kwargs["attention_mask"] = torch.ones_like(
            initial_input_ids
        )
    if "past_key_values" not in initial_model_kwargs:
        initial_model_kwargs["past_key_values"] = None
    ...
    while stack:
        ...
        input_ids = (
            ...
            if additional_tokens.numel() > 0
            else initial_input_ids
        )

        attention_mask = torch.cat(
            [
                initial_model_kwargs["attention_mask"],
                torch.ones_like(
                    initial_model_kwargs["attention_mask"][
                        :, : additional_tokens.shape[0]
                    ]
                ),
            ],
            dim=-1,
        ...
        if (
            all(stopping_list[0](input_ids, None))
            or (input_ids[:, -1] == stopping_list[1]).any()
        ):
        ...
        if depth == 0:
            ...
            new_logits_full[:, I_token] = (
                0  # set the probability of the "I" token to 0
            )
            new_logits_full[:, n_token] = (
                0  # set the probability of the "/n" token to 0
            )
            new_logits_full[:, Please_token] = (
                0  # set the probability of the "Please" token to 0
            )

        new_logits = new_logits_full[:-1, :]
        new_logits = torch.nn.functional.softmax(
            new_logits / temperature, dim=-1
        )
        no_retrieval_token = new_logits_full[-1].argmax()
        ...
        if top1_top2_gap > thres1:
            ...
            if no_retrieval_token != top1_index:
                logger.debug(
                    f"Zero-shot: {no_retrieval_token}, top1: {top1_index}, branch: {top1_top2_gap}"
                )
                selected_tokens_list.append(top1_index)
        ...
        for selected_token in selected_tokens_list:
            new_additional_tokens = (
                torch.cat(
                    [additional_tokens, selected_token.unsqueeze(0)], dim=0
                )
                if additional_tokens.numel() > 0
                else selected_token.unsqueeze(0)
            )
...
class DecodingAgg(RRAG):
    def __init__(
        ...
        llm,
        args,
        abstention_prob=None,
        robust_agg="mean",
        certify_save_path="",
        eval_certify=None,
    ):
        self.llm = llm
        self.llm.model.secure_decoding_certify = (
            secure_decoding_certify.__get__(
                self.llm.model, type(self.llm.model)
            )
        )
        self.llm.model.secure_decoding = secure_decoding.__get__(
            self.llm.model, type(self.llm.model)
        )
        self.temperature = 1.0  # args.temperature
        self.robust_agg = robust_agg
        abstention_prob_list = {
            "mistralai/Mistral-7B-Instruct-v0.2": 0.99,
            "meta-llama/Llama-2-7b-chat-hf": 0.99,
            "mistralai/Mixtral-8x7B-Instruct-v0.1": 0.999,
        }
        if abstention_prob is None:
            self.abstention_prob = abstention_prob_list.get(
                llm.model_name, 0.99
            )
            logger.debug(
                f"Using default abstention probability: {self.abstention_prob}"
            )

        self.args = args
        self.eta = args.eta
        self.subsample_iter = args.subsample_iter
        self.certify_save_path = certify_save_path  # we need to save all possible reponses for long-form generation certification
        self.eval_certify = (
            eval_certify if eval_certify is not None else self.eta == 0
        )  # when eta == 0, we do not use no-retrieval token -- we are usually doing QA task

    def preprocess_input(self, data_item):
        prompt_list = self.llm.wrap_prompt(
            data_item, as_multi_choice=False, seperate=True
        )
        data_item_zero_shot = {
        ...
        prompt_zero_shot = self.llm.wrap_prompt(
            data_item_zero_shot, as_multi_choice=False, seperate=False
        )
        prompt_list.append(prompt_zero_shot)

        prompt_list_draft = [
            prompt + " I don't know" for prompt in prompt_list
        ]

        # batched version
        input_dict_draft = self.llm.tokenizer(
            prompt_list_draft, return_tensors="pt", padding=True
        ).to("cuda")
        ...
        with torch.no_grad():
            output_token_draft = self.llm.model(
                input_ids_draft, attention_mask=attention_mask_draft
            )
        ...
        input_dict = self.llm.tokenizer(
            prompt_list, return_tensors="pt", padding=True
        )
        start_index = input_dict.input_ids.size(1)

        for i in range(
            start_index, input_ids_draft.size(1) - 1
        ):  # Exclude the last token since there's no next token to predict
            # Get the probability of the actual next token
            next_token_id = input_ids_draft[
                0, i + 1
    ...
    def query(self, data_item, corruption_size=1):
        ...
        stopping_criteria = StoppingCriteriaList(
            [
                MaxLengthCriteria(
                    max_length=len(input_ids[0]) + self.llm.max_output_tokens
                ),
                StopOnTokens(stop_token_ids=stop_token_ids),
            ]
        )

        generated_outputs = self.llm.model.secure_decoding(
            input_ids,
            attention_mask=attention_mask,
        ...
        generated_output_text = self.llm.tokenizer.decode(
            generated_outputs, skip_special_tokens=True
        )
        certify = False

        if corruption_size > 0:
            if self.eval_certify and (
                not self._eval_response(generated_output_text, data_item)
            ):
                pass
            else:
    ...
    def certify(
        ...
        if random_sample_once:
            no_sampling = False
        generated_outputs = []
        certify = True
        # print(no_sampling)
        for j in range(iterations):
            outputs_local, certify_local = (
                self.llm.model.secure_decoding_certify(
                    input_ids,
                    attention_mask=attention_mask,
                    stopping_criteria=stopping_criteria,
        ...
        if certify:
            response_list = [
                self.llm.tokenizer.decode(response, skip_special_tokens=True)
            ...
            if self.eval_certify:
                certify = all(
                    [
                        self._eval_response(response, data_item)
