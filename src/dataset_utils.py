...
from .helper import clean_str, load_json
...
class DataUtils:  # base class for dataset
    ...
    def process_data_item(
        ...
        if (
            add_expanded_answer
        ):  # using additional equivalent answers written by GPT (GPT can make mistakes occasionally)
            answer += data_item["expanded answer"]

        incorrect_answer = data_item.get(
            "incorrect answer", []
        )  # used for running targeted attack
        incorrect_context = data_item.get(
            "incorrect_context", []
        )  # used for running Poison attack

        if include_title:  # include webpage title or not
            topk_content = [
                x["title"] + "\n" + x["text"]
                for x in context
                if ("text" in x) and ("title" in x)
            ]
        else:
    ...
    def eval_response(self, response, data_item):  # eval the correctness of QA
        ...
        for ans in answer:
            if normalize_answer(ans) == response:  ## changed from in to == EM
                logger.debug("correct!")
                return True
        return False

    def eval_response_asr(
        self, response, data_item
    ):  # eval if the targeted attack succeeds
...
class RealtimeQA(DataUtils):
    ...
    def process_data_item(self, data_item, top_k=None):
        ...
        if self.as_multi_choices:
            ...
            ret.update({"choices": choices, "choices_answer": choices_answer})
        return ret
    ...
    def eval_response(self, response, data_item):
        if not self.as_multi_choices:
            ...
            if "Answer:" in response:
                response = response[
                    response.index("Answer:") + len("Answer:") :
                ]
...
class Biogen(DataUtils):
    ...
    def process_data_item(self, data_item, top_k=None):
        ret = super().process_data_item(data_item, top_k)
        ret.update(
            {"long_gen": data_item.get("long_gen", False)}
        )  # add a tag for long-form generation
        return ret
...
def load_data(dataset_name, top_k, data_path=None):
    data_path = (
        data_path if data_path else f'data/{dataset_name.split("-")[0]}.json'
    )
